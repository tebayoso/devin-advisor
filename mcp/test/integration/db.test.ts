...
describe("plans persistence", () => {
  it("inserts a plan and reads it back with round-tripped JSON", async () => {
    ...
    const fetched = await getPlan(env, plan.id, "ws-1");
  ...
  it("normalizes a null workspace to the default scope and keeps a null summary", async () => {
    const plan = await insertPlan(env, {
      workspace: null,
    ...
    expect(plan.workspace).toBe("default");
    const fetched = await getPlan(env, plan.id, null);
    expect(fetched?.workspace).toBe("default");
    expect(fetched?.confidenceSummary).toBeNull();
  });

  it("returns null for an unknown plan id", async () => {
    expect(await getPlan(env, "does-not-exist", null)).toBeNull();
  });
...
describe("memory persistence", () => {
  ...
  it("maps a null workspace to the default scope", async () => {
    ...
    const defaults = await queryMemory(env, null, "shared");
    expect(defaults).toHaveLength(1);
    expect(defaults[0].workspace).toBe("default");
  });
